...
@section('content')
    ...
    <div class="mb-8">
        ...
        {{-- Info wilayah user --}}
        @if($userProvince && Auth::user()->isUser())
            <div class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-purple-50 border border-purple-200 rounded-lg text-sm text-purple-700">
                ...
                Lokasi Anda terdeteksi di <span class="font-semibold">{{ $userProvince }}</span>
                @if($nearbyProvinces)
                    &mdash; buku dari wilayah terdekat ditampilkan lebih dulu
                @endif
            </div>
        @endif
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">
                        Region / Wilayah
                        @if($userProvince && (empty(request('region')) || request('region') === 'all') && Auth::user()->isUser())
                            <span class="ml-1 text-xs text-purple-500 font-normal">(terdekat dari Anda)</span>
                        @endif
                    </label>
                    <select name="region" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-black">
                        <option value="all" {{ (empty(request('region')) || request('region') === 'all') ? 'selected' : '' }}>
                            Semua Wilayah
                        </option>
                        @foreach($allProvinces as $prov)
                            <option value="{{ $prov }}"
                                {{ request('region') === $prov ? 'selected' : '' }}
                                @if($userProvince === $prov) data-near="same"
                                @elseif(in_array($prov, $nearbyProvinces)) data-near="nearby"
                                @endif>
                                {{ $prov }}
                                @if($userProvince === $prov) ★ Wilayah Anda
                                @elseif(in_array($prov, $nearbyProvinces)) · Sekitar
                                @endif
                            </option>
                        @endforeach
                    </select>
