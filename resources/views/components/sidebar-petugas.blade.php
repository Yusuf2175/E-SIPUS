...
<aside class="fixed left-0 top-0 h-screen w-64 bg-white border-r border-slate-200 overflow-y-auto z-40">
    ...
    <nav class="p-4 space-y-1">
        ...
        <div class="pt-4">
            ...
            <a href="{{ route('borrowings.index') }}" class="flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition {{ request()->routeIs('borrowings.*') ? 'bg-green-50 text-green-700' : 'text-slate-700 hover:bg-slate-50' }}">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
